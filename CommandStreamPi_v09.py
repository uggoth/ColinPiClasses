...
class Handshake(ColObjects.ColObj):
    ...
    def is_on(self):
        status = self.gpio.read(self.pin_no)
        if status == 1:
            return False
        else:
            return True
...
class Pico(ColObjects.ColObj):
    ...
    def __init__(self, pico_id, gpio, handshake):
        ...
        self.possible_ports = ['/dev/ttyACM0',
                               ...
                               '/dev/ttyACM2',
                               '/dev/ttyACM3',
                               '/dev/ttyUSB0',  ### Note USB00 on Bookworm
                               '/dev/ttyUSB1']
        self.id = pico_id
        self.gpio = gpio
    ...
    def send(self, text):  #  Don't use directly. Use send_command
        in_text = text + '\n'
    ...
    def send_command_and_wait(self, serial_no, command):
        sub_result_1 = self.send_command(serial_no, command)
        if self.handshake is not None:
            sub_result_2 = self.handshake.wait()
        return sub_result_1
